...
app.post("/submit/:regno/:id", upload.single("file"), async (req, res) => {
  ...
  const classs = await Classroom.findOne({ classname: student.classname });
  let assignment1 = classs.assignment;
  let profname

  for (i in assignment1) {
    console.log(i);
    console.log(assignment1[i]._id);

    if (assignment1[i]._id == id) {
      // console.log(i);
      // console.log(assignment1[i]._id);
      assignment1[i].submissions = [
        ...assignment1[i].submissions,
        { regno: regno, filename: filename, originalname: originalname },
      ];
      profname = assignment1[i].profname
      console.log(assignment1[i].submissions);
    }
  }

  await Classroom.findOneAndUpdate(
    { classname: student.classname },
    { $set: { assignment: assignment1 } }
  );
  await Prof.findOneAndUpdate(
    { name: profname },
    { $set: { assignment: assignment1 } }
