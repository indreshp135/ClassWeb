...
router.post('/join',async(req,res)=>{
    ...
    try{
        ...
        await Classroom.findOneAndUpdate({classname:classname},{$push:{subjects:subj1}});
...
//REQUEST TTO APPOINT A CR
router.post('/appoint',async(req,res)=>{
    const {regno} = req.body;
    try{
        const show =await Student.findOneAndUpdate({regno:regno},{CR:true})
...
router.post('/publish',async(req,res)=>{
    ...
    try{
        const result = {coursecode:coursecode,marks:marks,testtype:testtype}
        const show = await Student.findOneAndUpdate({regno:regno},{$push:{result:result}});
        if(!show)res.send(regno+' not yet registered ')
